  ...
  const loadData = async () => {
    ...
    try {
      ...
      try {
        ...
        setLlmConfig(llmConfigData ? normalizeLlmConfig(llmConfigData) : null)
      } catch (error) {
  ...
  const handleCreateOrUpdateLlmConfig = async () => {
    if (newLlmConfig.modelName.trim() && newLlmConfig.apiKey.trim()) {
      ...
      try {
        ...
        setLlmConfig(normalizeLlmConfig(savedConfig))
...
function normalizeLlmConfig(config: any): LlmConfig {
  return {
    id: config?.id,
    provider: config?.provider,
    modelName: config?.modelName || "gpt-4.1-mini",
    apiKey: config?.apiKey || config?.apiKeyEncrypted || "",
